...
// Chỉ cho phép Host có 1 trận đấu OPEN/FULL tại cùng một khung giờ.
// CANCELLED/COMPLETED không bị chặn để người dùng có thể tạo lại sau khi hủy.
...
* **Request Body:**
  ```json
  {
    ...
    "courtId": "6a0f022b22c105b435bb0e33", // Required (Bắt buộc phải chọn sân cụ thể)
...
class MatchingService {
  ...
  async createSession(data, hostId) {
    ...
    if (!data.courtId) {
      throw new Error('Bạn bắt buộc phải chọn sân cụ thể khi tạo phòng ghép trận');
    }
  ...
  async _findAvailableCourt(facilityId, sportId, bookingDate, startMinutes, endMinutes) {
    const Court = require('../models/court.model');
    const Booking = require('../models/booking.model');

    // 1. Lấy tất cả các sân thuộc facilityId và sportId đang hoạt động
    const courts = await Court.find({ facility_id: facilityId, sport_id: sportId, status: 'ACTIVE' });
    if (courts.length === 0) return null;

    // 2. Lấy các booking đã đặt trong khung giờ này
    ...
    // 3. Tìm sân chưa bị đặt
    for (const court of courts) {
      if (!bookedCourtIds.has(court._id.toString())) {
        return court._id; // Trả về sân trống đầu tiên
      }
    }
  ...
  async executeSuccessfulMatch(matchedQueues, sportId, facilityId, bookingDate) {
    ...
    // Tìm sân trống tự động cho nhóm ghép thành công này
    const courtId = await this._findAvailableCourt(facilityId, sportId, bookingDate, startMins, endMins);
    if (!courtId) {
      console.log(`[Matchmaker] Không tìm thấy sân trống phù hợp tại Facility ${facilityId} cho khung giờ ${startMins}-${endMins}. Tiếp tục giữ hàng chờ.`);
      return; // Không thể tạo session nếu không có sân trống phù hợp
    }
    ...
    for (const userId of allUserIds) {
      await notificationHelper.notifyUser({
        ...
        content: `Hệ thống đã tự động tìm thấy kèo đấu trùng khớp và chọn sân cho bạn vào ngày ${bookingDate} lúc ${Math.floor(startMins/60)}:00!`,
        type: 'SYSTEM',
        metadata: { matchingSessionId: session._id.toString() }
      });
    }

    console.log(`[Matchmaker] Ghép thành công Session ID: ${session._id} tại Court ID: ${courtId} cho các User: ${allUserIds.join(', ')}`);
  }
}
