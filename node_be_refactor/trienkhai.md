...
> [!IMPORTANT]
> Khi Host đăng tin tạo phòng ghép trận (`POST /api/v1/matching`), tham số `courtId` (ID của sân cụ thể) là **bắt buộc**. Giao diện UI khi tạo phòng ghép phải bắt người chơi chọn một sân cụ thể trong khu liên hợp.
