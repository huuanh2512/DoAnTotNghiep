...
class _AdminDashboardSectionState extends State<AdminDashboardSection> {
  ...
  Future<void> _handleLogout() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(context.tr(vi: 'Bạn có chắc chắn muốn đăng xuất?', en: 'Are you sure you want to log out?')),
        actions: [
          TextButton(
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        onTap: (index) {
          ...
          if (index == 2) {
            _loadFacilities(); // Tải lại cơ sở khi vào tab Sân đấu để đảm bảo dropdown cập nhật
          }
        },
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        context.tr(vi: 'Chào mừng tới trung tâm quản trị toàn hệ thống.', en: 'Welcome to the system administration center.'),
            ...
            Row(
              children: [
                _buildSystemStatCard(context.tr(vi: 'Tổng người dùng', en: 'Total Users'), _totalUsersCount.toString(), Icons.people_outline, Colors.blue),
                const SizedBox(width: 12),
                _buildSystemStatCard(context.tr(vi: 'Doanh thu hệ thống', en: 'Total Revenue'), _formatRevenue(_totalRevenue), Icons.auto_graph, Colors.green),
              ],
            ),
            ...
            GridView.count(
              ...
              children: [
                _buildAdminActionCard(
                  ...
                  context.tr(vi: 'Phân quyền, cấp phép tài khoản', en: 'Permissions & accounts setup'),
                  Icons.group_outlined,
                  Colors.indigo,
                  () => setState(() => _currentIndex = 4), // Chuyển trực tiếp sang Tab 4
                ),
                _buildAdminActionCard(
                  context.tr(vi: 'Quản lý Cơ sở', en: 'Manage Facilities'),
                  context.tr(vi: 'Quản lý các cơ sở sân đấu', en: 'Sports facilities setup'),
                  Icons.business,
                  Colors.orange,
                  () => setState(() => _currentIndex = 1), // Chuyển trực tiếp sang Tab 1
                ),
                _buildAdminActionCard(
                  context.tr(vi: 'Quản lý Sân đấu', en: 'Manage Courts'),
                  context.tr(vi: 'Danh sách sân đấu của các cơ sở', en: 'Courts list by facilities'),
                  Icons.stadium_outlined,
                  Colors.blue,
                  () => setState(() => _currentIndex = 2), // Chuyển trực tiếp sang Tab 2
                ),
                _buildAdminActionCard(
                  context.tr(vi: 'Môn thể thao', en: 'Sports'),
                  context.tr(vi: 'Danh mục môn thể thao hệ thống', en: 'System sports categories'),
                  Icons.sports_soccer_outlined,
                  Colors.red,
                  () => setState(() => _currentIndex = 3), // Chuyển trực tiếp sang Tab 3
                ),
              ],
            ),
            const SizedBox(height: 28),
              ...
              children: [
                Expanded(
                  child: SizedBox(
                    ...
                    child: _buildAdminActionCard(
                      context.tr(vi: 'Đặt lịch hệ thống', en: 'Booking Supervision'),
                      context.tr(vi: 'Giám sát đặt sân', en: 'Monitor court bookings'),
                      Icons.calendar_today_outlined,
                      Colors.teal,
                      () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => const AdminBookingSupervisionPage()),
                      ),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: SizedBox(
                    height: 110,
                    child: _buildAdminActionCard(
                      context.tr(vi: 'Thu chi & Giao dịch', en: 'Transactions & Payments'),
                      context.tr(vi: 'Duyệt giao dịch', en: 'Approve transactions'),
                      Icons.receipt_long_outlined,
                      Colors.purple,
                      () => Navigator.push(
                        context,
                        MaterialPageRoute(builder: (_) => const AdminPaymentSupervisionPage()),
                      ),
                    ),
                  ),
                ),
            ...
            SizedBox(
              ...
              child: _buildAdminActionCard(
                ...
                context.tr(vi: 'Quản lý và xóa các đánh giá của khách hàng', en: 'Manage and delete customer reviews'),
                Icons.rate_review_outlined,
                Colors.amber.shade800,
                () => Navigator.push(
                  context,
                  MaterialPageRoute(builder: (_) => const AdminModerationPage()),
                ),
              ),
            ),
            const SizedBox(height: 28),
            ...
            if (_recentBookings.isEmpty)
              _buildLogItem(context.tr(vi: 'Hệ thống', en: 'System'), context.tr(vi: 'Không có hoạt động đặt sân gần đây.', en: 'No recent booking activities.'), context.tr(vi: 'Bây giờ', en: 'Now'))
            else
              ..._recentBookings.map((booking) {
                final user = booking.user?.name ?? context.tr(vi: 'Khách hàng', en: 'Customer');
                final court = booking.courtName ?? context.tr(vi: 'Sân thi đấu', en: 'Court');
                final status = booking.status?.toUpperCase() ?? 'PENDING';
                String action = context.tr(vi: 'đã đặt sân $court.', en: 'booked court $court.');
                if (status == 'CANCELLED') {
                  action = context.tr(vi: 'đã hủy lịch đặt sân $court.', en: 'cancelled booking for court $court.');
                } else if (status == 'COMPLETED') {
                  action = context.tr(vi: 'đã hoàn thành trận đấu tại sân $court.', en: 'completed match on court $court.');
                } else if (status == 'CONFIRMED') {
                  action = context.tr(vi: 'đã xác nhận lịch đặt sân $court.', en: 'confirmed booking for court $court.');
                }
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(
                        context.tr(vi: 'Đăng xuất quản trị viên', en: 'Log out administrator'),
  ...
  Widget _buildCourtsTab() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              context.tr(vi: 'Chưa có cơ sở nào. Vui lòng tạo cơ sở trước.', en: 'No facilities found. Please create a facility first.'),
    ...
    if (_selectedFacilityId == null || !_facilities.any((f) => f.id == _selectedFacilityId)) {
      _selectedFacilityId = _facilities.first.id;
      _selectedFacilityName = _facilities.first.name;
    }
          ...
          child: Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                isExpanded: true,
                hint: Text(context.tr(vi: 'Chọn cơ sở để quản lý sân', en: 'Select facility to manage courts')),
                items: _facilities.map((fac) {
                  return DropdownMenuItem<String>(
                    value: fac.id,
                    child: Text(
                      fac.name ?? context.tr(vi: 'Cơ sở thể thao', en: 'Sports Facility'),
  ...
  Widget _buildAdminActionCard(String title, String desc, IconData icon, Color color, VoidCallback onTap) {
    final theme = Theme.of(context);
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(Icons.info_outline, size: 16, color: theme.colorScheme.primary.withValues(alpha: 0.6)),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
